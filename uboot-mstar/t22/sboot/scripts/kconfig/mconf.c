...
"Overview\n"
"--------\n"
"Some features may be built directly into sboot.\n"
...
static char buf[81920], *bufptr = buf;
...
static void show_textbox(const char *title, const char *text, int r, int c)
{
    ...
    retLen=write(fd, text, strlen(text));
    close(fd);
    if(retLen >= 0)
    {
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("SBoot %s Configuration"),
		sym_get_string_value(sym));

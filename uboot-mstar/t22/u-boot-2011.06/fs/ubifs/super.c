...
static int init_constants_early(struct ubifs_info *c)
{
    ...
    c->is_mlc = c->di.is_mlc;
#ifdef CONFIG_MTD_UBI_UBIFS_FS_REDUCE_MEM
	c->max_write_size = c->min_io_size * 8;
#else
	c->max_write_size = (c->leb_size + c->min_io_size * 2) / 4;
#endif
...
static int mount_ubifs(struct ubifs_info *c)
{
	...
	memset(c->bgt_name, 0 , sizeof(c->bgt_name));
...
static int ubifs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!c)
		return -ENOMEM;
	memset(c, 0, sizeof(struct ubifs_info));
...
{
	struct ubi_volume_desc *ubi=NULL;

...
#if defined(CONFIG_CMD_EXT4_WRITE) || defined(CONFIG_EXT4_JBD2_INIT)
void put_ext4(uint64_t off, void *buf, uint32_t size)
{
        ...
        if ((startblock + (size / SECTOR_SIZE)) >
            ...
            printf("error: overflow occurs %llu > %llu\n",
                startblock + (size / SECTOR_SIZE), part_offset + fs->total_sect);
            return;
        }
...
static int ext4fs_bsearch_index(struct ext4_extent_idx *ext, int low, int high, int target)
{
    if(ext == NULL)
        return -1;

    if((low > high) || (ext[low].ei_block > target))
        return -1;
    ...
    while(low < high)
    {
        if(ext[mid].ei_block < target)
            low = mid;
        else if(ext[mid].ei_block > target)
            high = mid -1;
        else
            return mid;
...
static int ext4fs_bsearch_extent(struct ext4_extent *ext, int low, int high, int target)
{
    if(ext == NULL)
        return -1;

    if((low > high) || (ext[low].ee_block > target))
        return -1;
    ...
    while(low < high)
    {
        if(ext[mid].ee_block < target)
            low = mid;
        else if(ext[mid].ee_block > target)
            high = mid -1;
        else
            return mid;
        ...
        uint32_t fileblock, int log2_blksz)
{
    ...
    while (1) {
        ...
        if (le32_to_cpu(ext_block->eh_magic) != EXT4_EXT_MAGIC){
            printf("EXT4_WARNING: invaild extent magic 0x%x, should be 0x%x!\n",
                        le32_to_cpu(ext_block->eh_magic), EXT4_EXT_MAGIC);
            return 0;
        }
        ...
        i = le32_to_cpu(ext_block->eh_entries)-1;
        if(i>=((fs->blksz-sizeof( struct ext4_extent_header))/sizeof (struct ext4_extent_idx)))
        {
            printf("EXT4_WARNING: invalid  ext4_extent_header1, i=%d\n",i);
            goto dumpbuf;
        }

        i = ext4fs_bsearch_index(index, 0, i, fileblock);
        if(i < 0)
        {
            printf("EXT4_WARNING: invalid  ext4_extent_header2, fileblock=%d, i=%d\n",
                fileblock, i);
            goto dumpbuf;
        }
             
        if (le32_to_cpu(index[i].ei_block) > fileblock){
            printf("EXT4_WARNING: invalid  ext4_extent_header3, fileblock=%d, i=%d\n",
                fileblock, i);
            goto dumpbuf;
        }
        ...
        if (ext4fs_devread(block << log2_blksz, 0, fs->blksz, buf))
            ...
            printf("EXT4_WARNING: read ext4_extent_header error\n");
            return 0;
        }
    }

//error and hole goto here
dumpbuf:
//    for(i=0; i<fs->blksz; i++)
//        printf("%02x ", ((char *)ext_block)[i]);
//    printf("\n");
    return 0;
...
long int read_allocated_block(struct ext2_inode *inode, int fileblock)
{
    ...
    if (le32_to_cpu(inode->flags) & EXT4_EXTENTS_FL) {
        ...
        if (!ext_block) {
            printf("EXT4_WARNING: invalid extent block at block %d\n", fileblock);
            free(buf);
            return -EINVAL;
        }
        ...
        i = le32_to_cpu(ext_block->eh_entries)-1;
        if(i>=((fs->blksz-sizeof( struct ext4_extent_header))/sizeof (struct ext4_extent)))
        {
            printf("EXT4_WARNING: invalid  ext4_extent_header\n");
            goto dumpbuf;
        }

        i = ext4fs_bsearch_extent(extent, 0, i, fileblock);
        if(i < 0)
        {
            printf("EXT4_WARNING: invalid  extent list case 0, fileblock=%d, i=%d\n",
                fileblock, i);
            goto dumpbuf;
        }
        
	 if(fileblock < le32_to_cpu(extent[i].ee_block))
        {
            printf("EXT4_WARNING: invalid  extent list case 1, fileblock=%d, i=%d\n",
                fileblock, i);
            goto dumpbuf;
        }
        ...
        if (fileblock >= le32_to_cpu(extent[i].ee_len)) {
            printf("EXT4_WARNING: invalid  extent list case 2, fileblock=%d, i=%d\n",
                fileblock, i);
		goto dumpbuf;
        }
               ...
               struct ext2fs_node **fnode, int *ftype)
{
    ...
    while (fpos < __le32_to_cpu(diro->inode.size)) {
        ...
        status = ext4fs_read_file(diro, fpos,
                      sizeof(struct ext2_dirent),
                      (char *)&dirent);
        if ((status < 1) || (dirent.namelen == 0) || (dirent.direntlen == 0))
            return 0;

...
{
        if (node && (node != &ext4fs_root->diropen) && (node != currroot))
            free(node);
}
                ...
                unsigned int len, char *buf)
{
        ...
        for (i = pos / blocksize; i < blockcnt; i++) {
            ...
            //if cannot get the block, we think it is in hole and return zero buffer.
            if (blknr < 0){
                printf("block in hole!\n");
                blknr = 0;
            }
...
int ext4fs_part_read(char *buf, int offset, unsigned len)
{
        int ret=0;
        ...
        ret = ext4fs_read_file(ext4fs_file, offset, len, buf);
...
#if defined(CONFIG_EXT4_JBD2_INIT)

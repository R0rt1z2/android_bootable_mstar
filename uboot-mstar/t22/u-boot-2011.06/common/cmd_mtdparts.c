...
#define SIZE_REMAINING		0xFFFFFFFFFFFFFFFFLL

/* special offset value, it is used when not provided by user
 *
 * this value is used temporarily during parsing, later such offests
 * are recalculated */
#define OFFSET_NOT_SPECIFIED	0xFFFFFFFFFFFFFFFFLL
 ...
 */
static unsigned long long memsize_parse (const char *const ptr, const char **retptr)
{
	unsigned long long ret = simple_strtoull(ptr, (char **)retptr, 0);
...
#define SIZE_KB ((u32)1024)
	...
	if ((size % SIZE_GB) == 0)
		sprintf(buf, "%llug", size/(u64)SIZE_GB);
	else if ((size % SIZE_MB) == 0)
		sprintf(buf, "%llum", size/(u64)SIZE_MB);
	else if (size % SIZE_KB == 0)
		sprintf(buf, "%lluk", size/(u64)SIZE_KB);
	else
...
#define PART_ADD_DESC_MAXLEN 64
		...
		if (!strcmp(&argv[1][3], ".spread")) {
			...
			debug("increased %s to %lld bytes\n", p->name, p->size);
		}
#endif
...
static int parse_dynparts(const char *const dynparts, char *mtdparts)
{
    ...
    char *size_suffix;
    u64 part_size;
    char *last_mtdparts=NULL;
    u8 u8_specify_offset = 0;
...
#if (ENABLE_MODULE_NAND_FLASH == 1) || (ENABLE_MODULE_SPI_NAND_FLASH == 1)
    #if (ENABLE_MODULE_NAND_FLASH == 1)
    drvNAND_GetMtdParts(nandmtdstr);
    #endif
    #if (ENABLE_MODULE_SPI_NAND_FLASH == 1)
    MDrv_SPINAND_GetMtdParts(nandmtdstr);
    #endif

    #if defined(CONFIG_ENV_IS_IN_NANDUBI)
    if(nandmtdstr[0] == 0)
    {
        printf("Please update \"PNI\" file to NAND Flash\n");
        printf("[%s] Fail\n",__func__);
        goto exit;
    }
    #endif
    ...
    if(nandmtdstr[0] != 0)
    {
    	//copy customer dynpart string to nandmtdstr
    	...
    	/* check dynparts length */
    	if (strlen(nandmtdstr) > MTDPARTS_MAXLEN)
    	{
    		printf("nandmtdstr length %d> 512\n", strlen(nandmtdstr));
    		goto exit;
    	}
    	//fetch "mtdparts="
    	mtd_id = p;
    	...
    	p++;			//pointer to the following char of "="
    ...
    while (p && (*p != '\0') && (*p != ';'))
    {
	   ...
	   u8_specify_offset = 1;			
        ...
        DEBUGF("%llX\t%llX\t%lld%s\t%s\n",part->offset, part->size, part_size, size_suffix, part->name);

	if(u8_specify_offset)
		sprintf(mtdparts, "%lld%s@0x%llX(%s),", part_size, size_suffix, part->offset, part->name);	
	else
	        sprintf(mtdparts, "%lld%s(%s),", part_size, size_suffix, part->name);
...
int do_dynpart(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    char *dynparts=NULL;
    char *mtdparts=NULL;
    int err;
...
#if (ENABLE_MODULE_NAND_FLASH == 1) || (ENABLE_MODULE_SPI_NAND_FLASH == 1)
	/* allocate temp buffer */
    nandmtdstr = malloc(MTDPARTS_MAXLEN);
    if (nandmtdstr == NULL)
    {
        printf("Error: malloc for temp buffer fail!!!\n");
        goto exit;
...
U_BOOT_CMD(
    ...
    "dynamically calculate partition table based on <mtd-def>/'"ENV_DYNPARTS"' and BBT (it is better to set the last partition size to SIZE_REMAINING)",
    "[<mtd-def>]\n"

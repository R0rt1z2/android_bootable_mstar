...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    case IMAGE_FORMAT_ANDROID:
        ...
        images.ep = images.os.load;
                ...
                if (!amzn_device_is_unlocked()) {
#else
                if (1) {
#endif
			/* Verify Amazon kernel signature */
			int image_size = (int)(images.os.end - images.os.start);

			if (!amzn_image_verify((uchar*)images.os.start,
				(uchar*)images.os.end, image_size, NULL)) {
				printf("Verify image failed\n");
				return 1;
			} else {
				printf("Verify image OK\n");
			}
		}
#endif
        break;
...
//Mstar Start
...
#if defined(CONFIG_ANDROID_BOOT) && (!defined(CONFIG_SECURE_BY_PASS_VERIFICATION))
#if defined(CONFIG_SECURITY_BOOT) || defined(CONFIG_ANDROID_AVB_ENABLE)

#if defined(CONFIG_SECURITY_BOOT)
    #include <secure/MsSecureBoot.h>
#elif defined(CONFIG_ANDROID_AVB_ENABLE)
    #include "libavb.h"
#endif
    #include <MsBoot.h>

    extern EN_BOOT_MODE IS_BOOTFLOW_FINISH;
    if(IS_BOOTFLOW_FINISH != EN_BOOT_MODE_UNKNOWN)
    {
        printf("IN cmd_bootm bootflow finish\n");
        if( IS_BOOTFLOW_FINISH == EN_BOOT_MODE_NORMAL )
        {
            //printf("IN cmd_bootm: EN_BOOT_MODE_NORMAL\n");
        }
        else if( IS_BOOTFLOW_FINISH == EN_BOOT_MODE_RECOVERY )
        {
            //printf("IN cmd_bootm: EN_BOOT_MODE_RECOVERY\n");
        }
        else
        {
            printf("IN cmd_bootm: should not be here\n\n");
        }
    }
    else
    {
        printf("IN cmd_bootm boot flow error\n");
#if defined(CONFIG_SECURITY_BOOT)
        Secure_HaltCPU();
#elif defined(CONFIG_ANDROID_AVB_ENABLE)
        avb_abort();
	...
	{
        #if defined(CONFIG_URSA_UNION) || defined(CONFIG_URSA20_VB1)
		...
		printf("put BT in woBLE mode \n");
		run_command("setMtkBT", 0);
	#endif
	    extern void MsDrv_PNL_BackLight_On(void);
		extern int is_quiescent_mode(void);
		if(!is_quiescent_mode()) {
			printf("Turn on backlight for recovery mode \n");
		...
		else
		{
			...
			if (IsTVOled()) // OLED panel still need to enable BL in Silent OTA, only mute Ursa.
			{
				...
				printf("Enable the panel VDD when Silent OTA mode for OLED Panel\n");
				MsDrv_PNL_BackLight_On();
			}
			else
#endif
			{
				printf("Silent OTA, do not turn on backlight\n");
			}
		}
	}
#endif
//Mstar End
	...
	/* setup uboot log in fdt */
	run_command("fdt addr 0x23000000", 0);
	run_command("fdt ubootlog", 0);
...
static int android_image_get_kernel(struct andr_img_hdr *hdr, int verify)
{
	...
	*newbootargs = '\0';
	...
	if (bootargs) {
		...
		if (*hdr->cmdline) {
			/* Only append cmdline while there is no same mboot environment parameter */
			cmdline_tmp_str = strstr(newbootargs, "buildvariant");
			if(cmdline_tmp_str != NULL) {
				strncpy (cmdline_tmp_str, hdr->cmdline, strlen(hdr->cmdline));
			}
			else {
				strcat(newbootargs, " ");
...
#endif
			/* Verify Amazon kernel signature */
			int image_size = image_get_data_size (hdr) + image_get_header_size();
			uchar* signature = (uchar*)img_addr + image_size;
			if (!amzn_image_verify((uchar*)img_addr, signature, image_size, NULL)) {
			        debug("Verify image failed\n");
				return NULL;
			}
			else {
			        printf("Verify image OK\n");

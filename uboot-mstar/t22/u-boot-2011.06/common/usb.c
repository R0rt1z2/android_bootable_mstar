    ...
    *((unsigned char volatile *)(0xBF200000+0x1200*2+0xF0*2+1)) =  (*((unsigned char volatile *)(0xBF200000+0x1200*2+0xF0*2+1))
    ...
    if(result==0) {
        ...
        if(result) {
            ...
            if(retry_cnt > 0) {
                printf("[USB] bad device retry, countdown:%d\n", retry_cnt);
                retry_cnt--;
                goto retry;
            }
...
{
	int i;
	/*USB_PRINTF*/printf("New Device %d\n", dev_index);

...
void env_add_bootargs(char* oldstr, char* newstr)
{
    char *p1 = NULL, *p2 = NULL;
    char *bootargs = NULL;
    int ori_bootargs_len = 0;
    ...
    ori_bootargs_len += strlen(p1);
    ori_bootargs_len += strlen(newstr);
    ori_bootargs_len += 2;

    bootargs = (char *)malloc(ori_bootargs_len);
    memset(bootargs, 0, ori_bootargs_len);
    strcpy(bootargs, p1);
...
int do_env_set(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if((argc >= 3) && (0==strcmp(argv[1], "ethaddr") || 0==strcmp(argv[1], "macaddr")))
    {
        if ( argc > 3
            || strlen(argv[2]) != strlen("XX:XX:XX:XX:XX:XX")
            || ':' != argv[2][2]
            || ':' != argv[2][5]
            || ':' != argv[2][8]
            || ':' != argv[2][11]
            || ':' != argv[2][14])
        {
            printf("\033[0;31m [ERROR] setenv ethaddr or macaddr format Fail!! \033[0m \n");
            printf("\033[0;31m Correct formt is XX:XX:XX:XX:XX:XX \033[0m \n");
            return -1;
        }
    }
...
int do_env_edit(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if (init_val != NULL)
    {
        if(strlen(init_val) > CONFIG_SYS_CBSIZE)
            buffer[0] = '\0';
        else
...
static void copy_env(char *dest, char *source, int max_size)
{
    if (source == NULL)
        return;
...
int do_clearn_all_env(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    else if(argc == 2)
    {
        ...
        {
            copy_env(ipaddr, getenv("ipaddr"), 64);
            copy_env(serverip, getenv("serverip"), 64);
            copy_env(ethaddr, getenv("ethaddr"), 64);
            copy_env(macaddr, getenv("macaddr"), 64);
            copy_env(hsf, getenv("hfs"), 64);
            copy_env(UpgradeImage, getenv(ENV_UPGRADE_NAME), 256);
        #if defined(CONFIG_ENV_IS_IN_NANDUBI)
            copy_env(mtdparts, getenv("mtdparts"), 512);
        #endif
            if(hdeleteall_r(&env_htab)<0)
            {
                return -1;
            ...
            setenv("hfs", hsf);
            setenv(ENV_UPGRADE_NAME, UpgradeImage);
        #if defined(CONFIG_ENV_IS_IN_NANDUBI)

...
typedef enum
{
    E_CONSOLE_LOG_OPEN  = 0,
    E_CONSOLE_LOG_CLOSE = 1,
}CONSOLE_STATUS;

static CONSOLE_STATUS console_stat = E_CONSOLE_LOG_OPEN;

void open_console_log(void)
{
    console_stat = E_CONSOLE_LOG_OPEN;
}

void close_console_log(void)
{
    console_stat = E_CONSOLE_LOG_CLOSE;
}

static int if_close_log(void)
{
	...
	{
		// In debug level, unconditionally open console log
		return 0;
	}

	if (console_stat == E_CONSOLE_LOG_CLOSE)
	{
		pval = getenv("close_log");
		if ((pval != NULL) && (strcmp(pval, "yes") == 0))
		{
			return 1;
		}
	}
...
void amzn_init_log_buf()
{
	amzn_log_buf   = malloc(AMZN_LOG_SIZE);
	amzn_log_begin = 0;
	amzn_log_end   = -1;
	amzn_log_init  = AMZN_LOG_MAGIC;
	printf("%s - buf:0x%08x, begin:%d, end:%d, init:0x%08x\n", __FUNCTION__,
		 amzn_log_buf, amzn_log_begin, amzn_log_end, amzn_log_init);
}
...
//CONFIG_UART2HDMI
#if defined(CONFIG_UART2HDMI)
    uartstr = getenv ("uart2hdmi");
    if(uartstr && !strcmp(uartstr,"on"))
    {
        printf("Switching UART to HDMI!!!\n");
        MDrv_UART_Init(E_UART_PIU_UART0,115200);
        mdrv_uart_connect_mux(E_UART_PIU_UART0, E_UART_PORT0, E_UART_PAD_0_1);
    }
    else if(uartstr && !strcmp(uartstr,"off"))
    {
        printf("Switching UART to console!!!\n");
        MDrv_UART_Init(E_UART_PIU_UART0,115200);
        mdrv_uart_connect_mux(E_UART_PIU_UART0, E_UART_PORT0, E_UART_PAD_0_0);
    }
#endif
//End CONFIG_UART2HDMI

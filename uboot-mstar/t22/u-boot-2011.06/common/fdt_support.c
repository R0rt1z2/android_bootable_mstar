...
int fdt_add_new_memory_banks(void *blob, u64 start[], u64 size[], int banks, char *node_name)
{
	int err, nodeoffset;
	...
	nodeoffset = fdt_path_offset(blob, node_path);
	if (nodeoffset < 0) {
		nodeoffset = fdt_add_subnode(blob, 0, *node_name);
		if (nodeoffset < 0)

...
#ifndef CONFIG_DTB_ADDR
#define CONFIG_DTB_ADDR 0x21600000
#endif
#endif
...
#if defined(CONFIG_FIT) || !defined(CONFIG_OF_LIBFDT) //TODO: change back for proper DTB work
    char *fit_hdr;
#endif
    if( img_addr != NULL )
    {
        ...
        uint8_t arch, ulong *rd_start, ulong *rd_end)
{
    ...
    *rd_end = 0;
    ...
    if ((argc >= 3) && (strcmp(argv[2], "-") ==  0)) {
        ...
        && (images->ahdr && images->ahdr->ramdisk_size) ){
...
int boot_ufdt_overlay(char **of_flat_tree, ulong *of_size)
{
    ulong dtbo_image_buffer = CONFIG_DTBO_ADDR;
    ...
    merged_fdt = *of_flat_tree;
    merged_fdt_size = *of_size;
...
#else // #ifdef CONFIG_SUPPORT_VERIFY_DTBO_BY_VTS

int boot_ufdt_overlay(char **of_flat_tree, ulong *of_size)
{
    ulong overlay_fdt_buffer = CONFIG_DTBO_ADDR;
    image_header_t *overlay_fdt_image_hdr = (image_header_t *)overlay_fdt_buffer;
    struct fdt_header *main_fdt = NULL;
    struct fdt_header *overlay_fdt = NULL;
    struct fdt_header *merged_fdt = NULL;
    ulong main_fdt_size = 0;
    ulong overlay_fdt_size = 0;
    ulong merged_fdt_size = 0;
    uint32_t take_time = 0;

    main_fdt = *of_flat_tree;
    main_fdt_size = *of_size;

    overlay_fdt = image_get_data(overlay_fdt_image_hdr);
    overlay_fdt_size = image_get_size(overlay_fdt_image_hdr);
    ...
    //take_time = get_timestamp();
    merged_fdt = ufdt_apply_overlay(main_fdt,main_fdt_size,overlay_fdt,overlay_fdt_size);
    //take_time = get_timestamp() - take_time;
    if (merged_fdt == NULL)
    {
        printf("%s:%d:%s: Error, get merged fdt fail!\n",__FILE__,__LINE__,__FUNCTION__);
...
#endif // #ifdef CONFIG_SUPPORT_VERIFY_DTBO_BY_VTS
#endif
        ...
        char **of_flat_tree, ulong *of_size)
{
    ...
    *of_size = 0;
    ...
    if (argc > 0 || genimg_has_config (images)) {
#if defined(CONFIG_FIT)
        if (argc > 3) {
            /*
        ...
        case IMAGE_FORMAT_LEGACY:
            ...
            fdt_hdr = images->os.image_len + images->os.image_start;
            if (!fdt_hdr)
                goto error;

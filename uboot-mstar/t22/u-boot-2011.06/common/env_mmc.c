			...
			unsigned long offset, const void *buffer)
{
    ...
    memset(env_part_name, 0, sizeof(env_part_name));
    #ifdef CONFIG_MTK_LOADER
    strncpy(env_part_name, EMMC_MPOOLA_PAR_NAME, 31);
    #else
    strncpy(env_part_name, "MPOOL", 31);
    #endif
    ...
    {
        printf("_write_env failed : can not find Env Partition(%s)\n", env_part_name);
        free(mpart);
        return -1;
    }
			...
			unsigned long offset, const void *buffer)
{
    ...
    if (i > EMMC_RESERVED_FOR_MAP_V2)
    {
        printf("_read_env failed : can not find Env Partition(%s)\n", env_part_name);
        free(mpart);
        return -1;
    }

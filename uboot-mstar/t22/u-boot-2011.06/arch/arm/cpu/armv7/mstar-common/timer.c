...
#define TIMER0_BASE 	0x1f006040
#define TIMER0_MAX_L 	0x1f006048
#define TIMER0_MAX_H 	0x1f00604C
...
ulong get_timer_gap(ulong base)
{
#if MSTAR_TIMER
	cnt = GET_CNT - base;
	//printf("timer_cnt = 0x%x\n", cnt);
	return cnt;
#else
	return get_timer_masked() - base;
#endif
...
void __udelay(unsigned long usec)
{
	...
	unsigned long tmp = 0;
	//The maximum for __udelay is 350 seconds
	if(usec <= 350 * 1000 * 1000)
	{
	    tmo = usec * ((M_TIMER_CLOCK / 1000) / 1000);
	}
	else
	{
	    tmo = 350 * 1000 * 1000 *  ((M_TIMER_CLOCK / 1000) / 1000);
	}

...
#endif
#if defined(CONFIG_MSTAR_UNFD_FTL) || defined(CONFIG_MSTAR_FTL)
...
void init_part (block_dev_desc_t * dev_desc)
{
	...
	if (test_part_ufs(dev_desc) == 0) {
		dev_desc->part_type = PART_TYPE_UFS;
		return;
	}
#endif
#endif
...
#ifdef CONFIG_UFS_PARTITION
	case PART_TYPE_UFS:
		if (get_partition_info_ufs(dev_desc,part,info) == 0) {
			PRINTF ("## Valid UFS partition found ##\n");
			return (0);
		}
		break;
#endif
...
#ifdef CONFIG_UFS_PARTITION
	case PART_TYPE_UFS:
		PRINTF ("## Testing for valid UFS partition ##\n");
		print_part_header ("UFS", dev_desc);
		print_part_ufs (dev_desc);
		return;
#endif

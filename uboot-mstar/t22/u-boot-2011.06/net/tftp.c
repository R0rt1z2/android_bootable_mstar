...
TftpSend(void)
{
	...
	case STATE_SEND_RRQ:
		...
		*s++ = htons(TFTP_RRQ);
		...
		sprintf((char *)pkt, "%lu", TftpTimeoutMSecs / 2000000);
		debug("send option \"timeout %s\"\n", (char *)pkt);
		pkt += strlen((char *)pkt) + 1;
#if (CONFIG_TFTP_TSIZE || CONFIG_MSTAR_RT_PM_IN_SPI)
	    ...
	    unsigned len)
{
	...
	case TFTP_OACK:
		...
		for (i = 0; i+8 < len; i++) {
            ...
            if(strcmp((char *)pkt+i, "tsize") == 0)
            {
                TftpImageSize = simple_strtoul((char *)pkt+i+6,
							   NULL, 10);
                //if image size is bigger than limited image size, it'll overwrite Uboot and make Uboot crash.
                //then you need split load image, load image more than one time.
        #if CONFIG_LZO
                if(TftpImageSize > ((CONFIG_UNLZO_DST_ADDR - load_addr) - gd->mon_len))
                {
                    printf("warning: image size is (0x%x), which is bigger than (0x%lx),pls consider to split the image.\n",
                        TftpImageSize,((CONFIG_UNLZO_DST_ADDR - load_addr) - gd->mon_len));
                }
        #else
                if(TftpImageSize > ((gd->ram_size - gd->mon_len)- load_addr))
                {
                    printf("warning: image size is (0x%lx), which is bigger than (0x%lx),pls consider to split the image.\n",
                        TftpImageSize,((gd->ram_size - gd->mon_len)- load_addr));
                }
        #endif
            }
     #endif
...
TftpTimeout(void)
{
	if (++TftpTimeoutCount > TftpTimeoutCountMax) {
		...
		NetStartAgain();
                ...
                /*
                 *    Mstar patch:
                 *    Retransmit a full file instead of sedding last-ack packet
                 */
		puts("\nTFTP Retransmit...\n");

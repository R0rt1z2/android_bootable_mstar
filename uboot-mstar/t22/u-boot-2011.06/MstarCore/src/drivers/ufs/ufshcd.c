...
#if 0 /* !defined(CONFIG_MSTAR_UFS) */
#include <linux/async.h>
#include <linux/devfreq.h>
#endif
#include "ufshcd.h"
#include "unipro.h"
#if 1 /* defined(CONFIG_MSTAR_UFS) */
...
#else
static int ufshcd_async_scan(void *data, async_cookie_t cookie);
#endif
...
static void ufshcd_exception_event_handler(struct ufs_hba *hba);
#endif
...
static int ufshcd_wait_for_intr_status(struct ufs_hba *hba, u32 mask, unsigned long timeout_ms)
{
	...
	while ((intr_status&mask) == 0) {
		udelay(interval_us);
		#if 1 // SAR5 detection
		if(ufs_mstar_pltfrm_pwrfail())
		{
			dev_err(hba->dev, "!");
			if (!ufshcd_clear_cmd(hba, 0))
			{
				/* successfully cleared the command, retry if needed */
				dev_err(hba->dev, "clear!\n");
			}
			else
			{
				dev_err(hba->dev, "clear timeout!\n");
			}
	...
	if ((intr_status&mask)) {
		...
		ufshcd_writel(hba, (intr_status&mask), REG_INTERRUPT_STATUS);
	}
	...
	if (intr_status) {
		...
		if (hba->errors)
		{
			...
			ufshcd_writel(hba, hba->errors, REG_INTERRUPT_STATUS);
		}
	}
	return err;
}
...
void ufshcd_send_command(struct ufs_hba *hba, unsigned int task_tag)
{
	...
	#if 1 /* defined(CONFIG_MSTAR_UFS) */
	ufs_mstar_pltfrm_flush_cache(hba->lrb[task_tag].utr_descriptor_ptr, sizeof(struct utp_transfer_req_desc));
	ufs_mstar_pltfrm_flush_cache(hba->lrb[task_tag].ucd_req_ptr, sizeof(struct utp_transfer_cmd_desc));
	#endif
...
ufshcd_wait_for_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	if(ufshcd_wait_for_intr_status(hba, UIC_COMMAND_COMPL, UIC_CMD_TIMEOUT) == 0)
		...
		ret = -ETIMEDOUT;
		#if 1 /* defined(CONFIG_MSTAR_UFS) */
		dev_err(hba->dev, "ufshcd_wait_for_uic_cmd %08X timeout!\n", uic_cmd->command);
		#endif
	}
 ...
 */
#ifdef CONFIG_MSTAR_UFS_DEBUG
const unsigned char uic_cmd_dme_str[0x20][0x20] =
{
	"NA",			// 0
	"DME_GET",		// 1
	"DME_SET",		// 2
...
__ufshcd_send_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	#ifdef CONFIG_MSTAR_UFS_DEBUG
	#if 0
	dev_err(hba->dev, "UIC CMD:%x,%x,%x,%x\n",
		uic_cmd->command, uic_cmd->argument1, uic_cmd->argument2, uic_cmd->argument3);
	#else
	dev_err(hba->dev, "UIC CMD:%s,%x,%x,%x\n",
	uic_cmd_dme_str[uic_cmd->command], uic_cmd->argument1, uic_cmd->argument2, uic_cmd->argument3);
	#endif
	#endif
...
RETRY:
	...
	case FORMAT_UNIT:
		timeout_ms = FORMAT_UNIT_TIMEOUT;
		break;
	case START_STOP:
		timeout_ms = START_STOP_TIMEOUT;
		break;
	case MOVE_MEDIUM:
		timeout_ms = MOVE_MEDIUM_TIMEOUT;
		break;
	case READ_ELEMENT_STATUS:
		timeout_ms = READ_ELEMENT_STATUS_TIMEOUT;
		break;
	case READ_DEFECT_DATA:
		timeout_ms = READ_DEFECT_DATA_TIMEOUT;
		break;
	default:
		timeout_ms = BLK_DEFAULT_SG_TIMEOUT;
		break;
	}

	err = ufshcd_wait_for_intr_status(hba, UTP_TRANSFER_REQ_COMPL, timeout_ms);
	if(!err)
	{
		err = cmd->result;
		if(err && lrbp->sense_buffer)
		{
			dumpMemory4("sense data : ", lrbp->sense_buffer, SCSI_SENSE_BUFFERSIZE);
		}
	}
	else if(err == -EAGAIN)
	{
		...
		struct ufshcd_lrb *lrbp, int max_timeout)
{
	...
	if (!time_left) {
		...
		if (!ufshcd_clear_cmd(hba, lrbp->task_tag))
			...
			dev_err(hba->dev, "%s: ufshcd_clear_cmd fail\n", __func__);
		#endif
	}
				...
				u32 size, bool ascii)
{
	...
	if (ascii) {
		...
		#if 1 /* defined(CONFIG_MSTAR_UFS) */
		dev_dbg(hba->dev, "Product Name :\n");
		#endif
		for(i=0 ; i<ascii_len ; i++)
		{
			buff_ascii[i] = buf[QUERY_DESC_HDR_SIZE+(i<<1)+1];
			#if 1 /* defined(CONFIG_MSTAR_UFS) */
			dev_dbg(hba->dev, "%02X ", buff_ascii[i]);
			#endif
		}
...
int ufshcd_write_configuration_desc(struct ufs_hba *hba, int desc_index, u8 *buf, u32 size)
{
	int ret;
	int buff_len;
	enum desc_idn desc_id = QUERY_DESC_IDN_CONFIGURAION;
	...
	ret = ufshcd_query_descriptor(hba, UPIU_QUERY_OPCODE_WRITE_DESC,
				      desc_id, desc_index, 0, buf,
				      &buff_len);

	if (ret || (buff_len > ufs_query_desc_max_size[desc_id]) ||
	  (buf[QUERY_DESC_LENGTH_OFFSET] > ufs_query_desc_max_size[desc_id])
	  || (buf[QUERY_DESC_DESC_TYPE_OFFSET] != desc_id)) {
		dev_err(hba->dev, "%s: Failed writing descriptor. desc_id %d buff_len %d ret %d\n",
...
int ufshcd_read_configuration_desc(struct ufs_hba *hba, int desc_index, u8 *buf, u32 size)
{
	return ufshcd_read_desc(hba, QUERY_DESC_IDN_CONFIGURAION, desc_index, buf, size);
}
...
static int ufshcd_memory_alloc(struct ufs_hba *hba)
{
	...
	if (!hba->utrdl_base_addr ||
	    (hba->utrdl_dma_addr & (PAGE_SIZE - 1))) {
		dev_err(hba->dev,
	...
	if (!hba->utmrdl_base_addr ||
	    (hba->utmrdl_dma_addr & (PAGE_SIZE - 1))) {
		dev_err(hba->dev,
...
static int ufshcd_uic_pwr_ctrl(struct ufs_hba *hba, struct uic_command *cmd)
{
	...
	if(cmd->command == UIC_CMD_DME_HIBER_ENTER)
	{
		if (hba->vops && hba->vops->suspend)
			hba->vops->suspend(hba, UFS_SYSTEM_PM);
	}
	else if(cmd->command == UIC_CMD_DME_HIBER_EXIT)
	{
		if (hba->vops && hba->vops->resume)
			hba->vops->resume(hba, UFS_SYSTEM_PM);
	}
	#endif
	spin_unlock_irqrestore(hba->host->host_lock, flags);
	if (ret) {
				...
				struct ufs_device_info *card_data)
{
	...
	#if 1 /* defined(CONFIG_MSTAR_UFS) */
	dev_dbg(hba->dev, "wManufacturerID = %04X\n", card_data->wmanufacturerid);	
	#endif
	...
	strlcpy(card_data->model, (const char *)(str_desc_buf + QUERY_DESC_HDR_SIZE),
...
void ufs_advertise_fixup_device(struct ufs_hba *hba)
{
	...
	memcpy(hba->model, card_data.model, sizeof(card_data));
 ...
 */
#if 1 /* defined(CONFIG_MSTAR_UFS) */
struct ufs_hba *pHostBusAdapter;
#endif
int ufshcd_alloc_host(struct device *dev, struct ufs_hba **hba_handle)
{
	...
	hba->host = host;
	#else
	pHostBusAdapter = kzalloc(sizeof(struct ufs_hba), GFP_KERNEL);
	if(!pHostBusAdapter) {
		dev_err(dev, "ufshcd_alloc_host failed\n");
		err = -ENOMEM;
		goto out_error;
	}
	hba = pHostBusAdapter;
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	#else
	err = ufshcd_async_scan((void *)hba, 0);
	#endif

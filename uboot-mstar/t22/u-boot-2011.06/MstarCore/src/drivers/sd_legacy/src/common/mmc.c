...
int switch_2_sdr104(struct mmc *mmc) // 100MB/s, max clock: 208 MHz
{
	...
	err = sd_switch(mmc, SD_SWITCH_SWITCH, 0, 3, (u8 *)&switch_status);
	if (err) return err;
...
int switch_2_ddr50(struct mmc *mmc) // 50MB/s, max clock: 50 MHz
{
	...
	err = sd_switch(mmc, SD_SWITCH_SWITCH, 0, 4, (u8 *)&switch_status);
	if (err) return err;
...
int switch_2_sdr50(struct mmc *mmc) // 50MB/s, max clock: 100 MHz
{
	...
	err = sd_switch(mmc, SD_SWITCH_SWITCH, 0, 2, (u8 *)&switch_status);
	if (err) return err;
...
int mmc_startup(struct mmc *mmc)
{
	...
	{
		printf("Find SDHC %llu.%llu GB [%llXh]\n", mmc->capacity>>30, (mmc->capacity&0x3FFFFFFFULL)>>20, mmc->capacity>>9);
		//static char *gStrSpeed[] = {"SDR12", "SDR25", "SDR50", "DDR50", "SDR104"};
	}
	else
	{

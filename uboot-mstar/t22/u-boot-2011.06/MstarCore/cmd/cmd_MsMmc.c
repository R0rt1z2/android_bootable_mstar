...
U_BOOT_CMD(
  ...
  "comments: if pad=0, dump the valid data of user area, FORCT.DEF & LEAP.DEF which are used for mass production.\n"
  "          if [offset/partitionname] & [dumpsize] equal to 0 or NULL. dump entire emmc include boot1, boot2, user area.\n"
);
#endif

U_BOOT_CMD(
  flashbin, 5, 1, do_flash_bin,
  "flashbin - dump/restore emmc or nand flash data to/from flash.bin\n",
  "command: flashbin [binname] [dump/restore] [valid] [partitionname]\n"
  "comments: 1. [valid] has effect on flash.bin when dump flash with the last partition:\n"
  "             when valid=1, only dump the last partition's valid data.\n"
  "             when valid=0, dump the last partition's entire data.\n"
  "          2. when dump flash without the last partition or restore flash, [valid] has no effect whether 1 or 0.\n"
  "          3. [partitionname] is a optional parameter, default is the whole flash without [partitionname].\n"
  "          4. when dump flash, flash.bin should be saved as ntfs3g/exfat supported file.\n"
